...
export const PLANS = {
  monthly: {
    ...
    cap: 760,
    // Video-editor pool grant (separate from the video-gen `cap`).
    // 1,500 cr = 50 min of auto-editing at 30 cr/min source. Editor
    // cost target ≤ $1 at full burn.
    videoEditorCap: 1500,
  },
  pro: {
    name: 'Haelabs Pro',
    ...
    // credit math.
    cap: 2280,
    // 4,500 cr = 150 min of auto-editing. Editor cost target ≤ $3.
    videoEditorCap: 4500,
  },
  yearly: {
    name: 'Haelabs Yearly',
    ...
    cap: 3840,
    // 7,500 cr/yr = 250 min/yr of auto-editing (~21 min/mo). Editor
    // cost target ≤ $5/yr at full burn. Yearly is smaller-pool /
    // cheaper-per-credit, matching the existing loyalty pattern.
    videoEditorCap: 7500,
  },
};
 ...
 * (not subscriptions). Three distinct kinds:
 *
 ...
 *   - kind: 'video-editor' — credits the `videoEditorCreditsRemaining`
 *     pool used by the long-form Video Editor (auto-cut, captions,
 *     transitions, auto-zoom). 30 cr per minute of source video at
 *     ~90% margin against AssemblyAI + Fly worker costs.
 *
 * The three pools never mix: a customer who buys an image pack cannot
 * spend those credits on video features, etc.
 */
export const TOPUPS = {
  ...
  // ── Video-editor credit packs (AI auto-editor — kie.ai-independent,
  // billed against AssemblyAI + Fly worker costs). Same 152 cr/$1
  // retail rate as the other pools. 30 cr buys 1 minute of source.
  've-s': {
    kind: 'video-editor',
    name: 'Haelabs Video Editor Credits (760)',
    lookupKey: 'faceforge_ve_topup_s_v1',
    amountCents: 500, // $5.00 → ~25 min of editing
    credits: 760,
  },
  've-m': {
    kind: 'video-editor',
    name: 'Haelabs Video Editor Credits (2,280)',
    lookupKey: 'faceforge_ve_topup_m_v1',
    amountCents: 1500, // $15.00 → ~76 min
    credits: 2280,
  },
  've-l': {
    kind: 'video-editor',
    name: 'Haelabs Video Editor Credits (7,600)',
    lookupKey: 'faceforge_ve_topup_l_v1',
    amountCents: 5000, // $50.00 → ~253 min
    credits: 7600,
  },
  've-xl': {
    kind: 'video-editor',
    name: 'Haelabs Video Editor Credits (15,200)',
    lookupKey: 'faceforge_ve_topup_xl_v1',
    amountCents: 10000, // $100.00 → ~506 min
    credits: 15200,
  },
};

// Video-editor credit rate. 30 cr per minute of source video = 0.5
// cr/sec. At 152 cr/$1 retail, that's $0.20/min sold against
// ~$0.02/min cost (AssemblyAI Universal + Fly + Anthropic + storage)
// → ~90% margin. Used by /api/video/render to compute the credit
// charge upfront from source duration.
export const VIDEO_EDITOR_CR_PER_MIN = 30;
export const VIDEO_EDITOR_CR_PER_SEC = VIDEO_EDITOR_CR_PER_MIN / 60;
...
export const PLAN_SURFACES = {
  monthly: {
    ...
    'video-editor': {
      lookupKey: 'faceforge_monthly_video_editor_v1',
      productName: 'Haelabs Video Editor Monthly',
    },
  },
  pro: {
    ...
    'video-editor': {
      lookupKey: 'faceforge_pro_video_editor_v1',
      productName: 'Haelabs Video Editor Pro',
    },
  },
  yearly: {
    ...
    'video-editor': {
      lookupKey: 'faceforge_yearly_video_editor_v1',
      productName: 'Haelabs Video Editor Yearly',
    },
  },
};
